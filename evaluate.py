...
class SemDistCalculator:
    ...
    def safe_semdist(self, sim: float, eps: float = 1e-4):
        ...
        return 0.0 if semdist < eps else round(semdist.item(), 5)
...
semdist = SemDistCalculator("xlm-roberta-base", alpha = 1000)
...
output_df = pd.read_csv('speech-accent-archive_transcriptions_small.csv')
...
results_df.to_csv("eva_small.csv", index=False)
